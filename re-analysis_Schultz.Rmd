...
Among the options for analyzing count data, the Poisson distribution is rather straightforward. If we expect, on average, $\lambda_t$ individuals m^-2^ in time period $t$, and we sample a total of $A$ m^2^, then each of $i$ counts during that period ($c_{i,t}$) can me modeled as
...
Another alternatve is the negative binomial distribution, which has an additional parameter to account for overdispersion in the data. Specifically,
...
$$
which means the variance converges to the mean as $r \rightarrow \infty$, whereby the negative binomial converges to the Poisson.
