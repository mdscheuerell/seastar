...
b_fit <- glm(before ~ 1, data = stars_wide, family = poisson(link = "log"))
summary(b_fit)
b_mean <- exp(sum(coef(b_fit)))/area
b_mean

## estimated mean density after
a_fit <- glm(after ~ 1, data = stars_wide, family = poisson(link = "log"))
summary(a_fit)
...
```{r, plot_rates, fig.height=4, fig.width=4, warning=FALSE, message=FALSE}
## base graphics
...
## ggplot
colnames(pdat) <- c("lo","med","hi")
pdat <- as.data.frame(pdat)
pdat$time <- c("Before","After")
ggplot(pdat, aes(x=time, y=med)) +
