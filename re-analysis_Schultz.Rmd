...
model {
  ## PRIORS
  r_bef ~ dnorm(0,0.001) T(0,);
  r_aft ~ dnorm(0,0.001) T(0,);
...
#                            prior = normal(0, 1), prior_intercept = normal(0, 10), prior_aux = normal(0, 10),
...
## before
## convert from log to normal space
#round(exp(coef(nb_fit_stan_bef))/area, 2)
summary(nb_fit_stan_bef)

## after
## convert from log to normal space
#round(exp(coef(nb_fit_stan_aft))/area, 3)
summary(nb_fit_stan_aft)
```

Here are histograms of the posterior samples (left) and a summary of the median and 95% credible interval of the posterior samples (right).

```{r plot_nb_fit_stan, fig.height=4, fig.width=7, message=FALSE, eval=FALSE}
## gather posteriors samples
pdat <- data.frame(Time = rep(c("before","after"), ea=mcmc_samp),
                   samples = exp(c(as.matrix(nb_fit_stan_bef),
                                   as.matrix(nb_fit_stan_aft)))/area)
## summary of posteriors
pdat2 <- pdat %>%
  group_by(Time) %>%
  summarize(lo = quantile(samples, 0.025),
