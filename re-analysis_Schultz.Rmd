...
## convert from log to real space
round(lambda_aft <- exp(coef(po_fit_glm_aft))/area, 2)
```
...
```{r pois_fit_stan, message=FALSE}
## before
po_fit_stan_bef <- stan_glm(before ~ 1, data = stars_wide, family = poisson(link = "log"))
print(po_fit_stan_bef)
plot(po_fit_stan_bef, plotfun = "areas", prob = 0.95)
## after
po_fit_stan_aft <- stan_glm(after ~ 1, data = stars_wide, family = poisson(link = "log"))
print(po_fit_stan_aft)
plot(po_fit_stan_aft, plotfun = "areas", prob = 0.95)
```
